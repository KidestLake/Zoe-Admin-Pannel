            ...
            <div class="col-md-12">
                ...
                <div class="card card-primary card-outline card-outline-tabs">
                    <div class="card-header p-2">
                        ...
                        <li class="nav-item">
                          <a class="nav-link active" id="active-client-tab" data-toggle="pill" href="#active-client" role="tab" aria-controls="active-client" aria-selected="true"> <span class="fa fa-list"> </span> Active Clients</a>
                        </li>

                        <li class="nav-item">
                            <a class="nav-link" href="/User/deactivatedClients/0" role="tab" aria-controls="custom-tabs-four-profile" aria-selected="false"> <span class="fa fa-list"> </span> Blocked Clients </a>
                        </li>
                      </ul>
                    </div>
                    <div class="card-body">
                      <div class="tab-content">
                        ...
                        <div class="tab-pane active" id="active-client">
                            ...
                            <table id="activeClients" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($activeClients as $key => $value)
                                        <tr id="tr{{$value['id']}}">
                                            <td>{{$offset+$key+1}}</td>
                                            ...
                                            <td>
                                                <div class='dropdown'>
                                                    ...
                                                    <ul class='dropdown-menu dropdown-menu-right p-3'>
                                                        <li><a href='#' class="text-secondary" data-toggle="modal" data-target="#deactivateClientModal{{$value['id']}}"> <span class="fas fa-times mr-2"></span> Block</a></li>
                                        ...
                                        <div class="modal fade" tabindex="-1" role="dialog" id="deactivateClientModal{{$value['id']}}">
                                            ...
                                            <div class="modal-dialog">
                                                ...
                                                <div class="modal-footer float-right">
                                                    <button class="btn btn-danger" onclick="deactivateClient({{$value['id']}})">Block</button>
                              ...
                              <div class="row d-flex justify-content-end">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination">
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a class="page-link" href="{{ url('User/activeClients/'.($offset-$limit).'/'.($pageNumber-1)) }}" aria-label="Previous">
                                                <span aria-hidden="true">&laquo;</span>
                                                <span class="sr-only">Previous</span>
                                                </a>
                                            </li>
                                        @endif
                                        @if ($pageNumber > 3)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/activeClients/'.($offset-($limit*3)).'/'.($pageNumber-3)) }}">{{$pageNumber-3}}</a></li>
                                        @endif
                                        @if ($pageNumber > 2)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/activeClients/'.($offset-($limit*2)).'/'.($pageNumber-2)) }}">{{$pageNumber-2}}</a></li>
                                        @endif
                                        @if ($pageNumber > 1)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/activeClients/'.($offset-($limit)).'/'.($pageNumber-1)) }}">{{$pageNumber-1}}</a></li>
                                        @endif

                                        <li class="page-item active"> <a class="page-link" >{{$pageNumber}} <span class="sr-only">(current)</span></a></li>

                                        @if (($offset + $limit) < $totalActiveClients)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/activeClients/'.($offset+($limit)).'/'.($pageNumber+1)) }}">{{$pageNumber+1}}</a></li>
                                        @endif
                                        @if (($offset + (2*$limit)) < $totalActiveClients)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/activeClients/'.($offset+($limit*2)).'/'.($pageNumber+2)) }}">{{$pageNumber+2}}</a></li>
                                        @endif
                                        @if (($offset + (3*$limit)) < $totalActiveClients)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/activeClients/'.($offset+($limit*3)).'/'.($pageNumber+3)) }}">{{$pageNumber+3}}</a></li>
                                        @endif

                                        @if ((($offset+$limit) == $totalActiveClients) || (($offset+$limit) > $totalActiveClients))
                                            <li class="page-item disabled">
                                                <a class="page-link" href="#" tabindex="-1">
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a class="page-link" href="{{ url('User/activeClients/'.($offset+$limit).'/'.($pageNumber+1)) }}" aria-label="Next">
                                                <span aria-hidden="true">&raquo;</span>
                                                <span class="sr-only">Next</span>
                                                </a>
                                          </li>
        ...
        function deactivateClient(id)
        {
            ...
            $.ajax({
                ...
                success:function(response) {
                    if(response){
                        $('#deactivateClientModal'+id).modal('hide');
