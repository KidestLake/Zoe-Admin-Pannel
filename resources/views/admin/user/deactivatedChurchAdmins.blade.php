...
@section('contentSection')
            ...
            <div class="col-md-12">
                ...
                <div class="card card-primary card-outline card-outline-tabs">
                    <div class="card-header p-2">
                        ...
                        <li class="nav-item">
                          <a class="nav-link active" id="deactivated-churchAdmins-tab" data-toggle="pill" href="#deactivated-churchAdmins" role="tab" aria-controls="deactivated-churchAdmins" aria-selected="true"> <span class="fa fa-list"> </span> Blocked Church Admins</a>
                        </li>
                    ...
                    <div class="card-body">
                      <div class="tab-content">
                        ...
                        <div class="tab-pane active" id="deactivated-churchAdmins">

                            <table id="deactivatedChurchAdmins" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                        ...
                                        @foreach ($deactivatedChurchAdmins as $key => $value)
                                        <tr id="tr{{$value['id']}}">
                                            <td>{{$offset+$key+1}}</td>
                                            ...
                                            <td>
                                                <div class='dropdown'>
                                                    ...
                                                    <ul class='dropdown-menu dropdown-menu-right p-3'>
                                                        <li><a href='#'  class="text-secondary" data-toggle="modal" data-target="#activateChurchAdminModal{{$value['id']}}"> <span class="fas fa-check mr-2"></span> Activate</a></li>
                                        ...
                                        <div class="modal fade" tabindex="-1" role="dialog" id="activateChurchAdminModal{{$value['id']}}">
                                            ...
                                            <div class="modal-dialog">
                                                ...
                                                <div class="modal-footer float-right">
                                                    <button onclick="activateChurchAdmin({{$value['id']}})" class="btn btn-primary">Activate</button>
                                                    <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                              ...
                              <div class="row d-flex justify-content-end">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination">
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset-$limit).'/'.($pageNumber-1)) }}" aria-label="Previous">
                                                <span aria-hidden="true">&laquo;</span>
                                                <span class="sr-only">Previous</span>
                                                </a>
                                            </li>
                                        @endif
                                        @if ($pageNumber > 3)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset-($limit*3)).'/'.($pageNumber-3)) }}">{{$pageNumber-3}}</a></li>
                                        @endif
                                        @if ($pageNumber > 2)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset-($limit*2)).'/'.($pageNumber-2)) }}">{{$pageNumber-2}}</a></li>
                                        @endif
                                        @if ($pageNumber > 1)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset-($limit)).'/'.($pageNumber-1)) }}">{{$pageNumber-1}}</a></li>
                                        @endif

                                        <li class="page-item active"> <a class="page-link" >{{$pageNumber}} <span class="sr-only">(current)</span></a></li>

                                        @if (($offset + $limit) < $totalDeactivatedChurchAdministrators)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset+($limit)).'/'.($pageNumber+1)) }}">{{$pageNumber+1}}</a></li>
                                        @endif
                                        @if (($offset + (2*$limit)) < $totalDeactivatedChurchAdministrators)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset+($limit*2)).'/'.($pageNumber+2)) }}">{{$pageNumber+2}}</a></li>
                                        @endif
                                        @if (($offset + (3*$limit)) < $totalDeactivatedChurchAdministrators)
                                            <li class="page-item"><a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset+($limit*3)).'/'.($pageNumber+3)) }}">{{$pageNumber+3}}</a></li>
                                        @endif

                                        @if ((($offset+$limit) == $totalDeactivatedChurchAdministrators) || (($offset+$limit) > $totalDeactivatedChurchAdministrators))
                                            <li class="page-item disabled">
                                                <a class="page-link" href="#" tabindex="-1">
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a class="page-link" href="{{ url('User/deactivatedChurchAdministrators/'.($offset+$limit).'/'.($pageNumber+1)) }}" aria-label="Next">
                                                <span aria-hidden="true">&raquo;</span>
                                                <span class="sr-only">Next</span>
                                                </a>
                                          </li>
    ...
    <script type="text/javascript">
        ...
        function activateChurchAdmin(id)
        {
            $id=id;
            $.ajax({
                ...
                success:function(response) {
                    if(response){
                        $('#activateChurchAdminModal'+id).modal('hide');
