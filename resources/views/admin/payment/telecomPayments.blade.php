...
@section('contentSection')
            ...
            <div class="col-md-12">
                ...
                <div class="card card-primary card-outline card-outline-tabs">
                    <div class="card-header p-2">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" id="payedPayment-tab" data-toggle="pill" href="#payedPayment" role="tab" aria-controls="payedPayment" aria-selected="true"> <span class="fa fa-list"> </span> Telecom Payment(Payed)</a>
                        </li>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $.ajax({
                url: "{{ url('payments/telecomPaymentsNotPayed/0') }}",
                type: 'get',
            ...
            $.ajax({
                url: "{{ url('payments/telecomPaymentsPayed/0') }}",
                type: 'get',
                success: function(data) {
        ...
        function loadMoreTelecomNotPayed(offsetVal, pageNumberVal) {
            $('#not_payed_table_data').html(loading);
            $.ajax({
                url: "{{ url('payments/telecomPaymentsNotPayed/') }}" + '/' + offsetVal + '/' + pageNumberVal,
                type: 'get',
                success: function(data) {
                    $('#not_payed_table_data').html(data);
                    $('#telecomPaymentNotPayed').DataTable({
        ...
        function loadMoreTelecomPayed(offsetVal, pageNumberVal) {
            $('#payed_table_data').html(loading);
            $.ajax({
                url: "{{ url('payments/telecomPaymentsPayed/') }}" + '/' + offsetVal + '/' + pageNumberVal,
                type: 'get',
                success: function(data) {
                    $('#payed_table_data').html(data);
                    $('#telecomPaymentPayed').DataTable({
        ...
        searchFieldNotPayed.addEventListener("keydown", function(e) {
            ...
            if (e.keyCode === 13) { //checks whether the pressed key is "Enter"
                searchForTNotPayed(searchInput);
            }
        });

        function searchForTNotPayed(searchInput) {
            if (searchInput != "") {

                $('#not_payed_table_data').html(loading);

                $.ajax({
                    url: "{{ url('payments/searchTNotPayed') }}" + '/' + searchInput,
                    type: 'get',
                    success: function(data) {
                        $('#not_payed_table_data').html(data);
        ...
        searchFieldPayed.addEventListener("keydown", function(e) {
            ...
            if (e.keyCode === 13) { //checks whether the pressed key is "Enter"
                searchForTPayed(searchInput);
            }
        });

        function searchForTPayed(searchInput) {
            if (searchInput != "") {

                $('#payed_table_data').html(loading);

                $.ajax({
                    url: "{{ url('payments/searchTPayed') }}" + '/' + searchInput,
                    type: 'get',
                    success: function(data) {
                        $('#payed_table_data').html(data);
